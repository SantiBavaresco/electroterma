...
import { Lavable } from "@/public/svg/lavable";
...
export const DiferencialLavable : React.FC<Props> =  ({size, text, textSize, customWith}) => {
	return (
		<>
			<main className="relative pl-4 flex justify-start items-center  w-full">
				<span className="z-10 ">
					<Lavable size={size || 70}/>
				...
				*/}
				...
				<p className={`z-0  h-2/3 pl-8 pr-2 md:px-10 py-1 -translate-x-8 
					...
					w-full
					`}>
					<span className="w-full px-2 py-1  flex justify-start items-center">{text || "Lavable"}</span>
				</p>
